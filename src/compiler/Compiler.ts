...
export function compileShader(graph: ShaderGraph, target: ICompilerTarget) {
    ...
    const compileGraph = (endpointType: NodeType, isVertex: boolean): string => {
        ...
        if (endpointNode) {
            const traverseBackwards = (currentNodeId: string) => {
                ...
                node.inputs.forEach(input => {
                    ...
                    if (connection) traverseBackwards(connection.sourceNodeId);
                });
            ...
            traverseBackwards(endpointNode.id);
        }
    ...
    return {
        vertexShader: compileGraph('OUTPUT_VERT', true),
        fragmentShader: compileGraph('OUTPUT_FRAG', false)
    };
}